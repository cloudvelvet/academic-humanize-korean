...
- AI 탐지 우회는 하지 않으며, 요청 시 AI 문체 위험 신호를 점검하고 실제 학술적 결함만 정리
...
AI 문체 점검과 정리를 함께 요청할 수 있습니다. 이 기능은 원고의 작성 주체를 판정하거나 확률을 산출하지 않고, 기계적 반복·모호한 귀속·근거 없는 단정·인용 이상처럼 학술적으로 검토할 신호를 제시합니다. 정리를 요청하면 확인된 학술적 결함만 수정하며 탐지기 통과를 보장하지 않습니다.
...
아래 원고에서 AI 티가 나는 기계적 반복과 상투적 연결을 정리해줘.
인용·수치·전문용어·주장 강도는 유지하고 탐지기 우회는 목표로 하지 마.
...
- `references/ai-writing-review.md` — 요청 시 적용하는 AI 문체 위험 신호, 정리 원칙과 출력 기준
